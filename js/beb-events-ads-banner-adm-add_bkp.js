jQuery(document).ready(function($){
	...
	var beb_eab_dominio = beb_eab_url_tmp [0] + '//' + beb_eab_url_tmp [2] + '/';
	if (beb_eab_url_tmp [2] == 'localhost') {
		beb_eab_dominio = beb_eab_dominio + beb_eab_url_tmp [3] + '/';
	}
	...
	// automatically opens media gallery in new window
	if ('' !== $('#beb-eab-immagine').val()) {
		$( ".contenitore-input.beb-eab-add-options-el" ).css('float', 'left');
		$( ".contenitore-input.beb-eab-add-texts-el, .contenitore-input.beb-eab-add-options-el" ).css('top', '0').css('left', '0px').css('margin', '250px 0 0 1%');
		$( ".contenitore-input.beb-eab-add-links-el" ).css('top', '-240px').css('left', '0px');	
		$( ".contenitore-input.beb-eab-add-media-el" ).css({'top': '0', 'margin': '1% 0'});
		beb_eab_check_imm = false;
		beb_eab_immagine_inserita = true;
		img.src = $('#beb-eab-immagine').val();
	} else {
		$('#beb-eab-immagine').one('focus', function(){
			var beb_width = (screen.width*70)/100;
			var left_window = (screen.width*15)/100;
			var beb_height = (screen.availHeight*60)/100;
			var top_window = (screen.availHeight*20)/100;
			window.open(beb_eab_dominio + 'wp-admin/upload.php', '_blank', 'width=' + beb_width + ', height=' + beb_height +
					', left=' + left_window + ', top=' + top_window);
		});
	}
	...
	$('#beb-eab-immagine').keyup(function() {
		...
		if (beb_eab_url_imm === '' && beb_eab_contatore2 === 1 && beb_eab_immagine_inserita === true) {
			...
			if (false === beb_eab_check_imm) {
				...
				beb_eab_larg_cont_banner = parseInt(beb_eab_larg_cont_banner) - beb_eab_imm_larg;
				beb_eab_larg_contenuto = parseInt(beb_eab_larg_contenuto) - beb_eab_imm_larg;
				beb_eab_larg_banner = parseInt(beb_eab_larg_banner) - beb_eab_imm_larg;
